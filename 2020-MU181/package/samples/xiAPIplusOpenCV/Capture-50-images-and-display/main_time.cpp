...
string get_filename(int exposure)
{
    string filename;
    filename += get_string_time() + "_exposure-" + to_string(exposure);
    return filename;
}
...
//			string filename_s = get_filename(EXPOSURE);
//			char* filename = const_cast<char*>(filename_s.c_str());
//			string filename = get_filename(EXPOSURE);
//			if (keyboard_input == 32) {
//				cv::imwrite(cv::strng(filename_s), cv_mat_image);
//				std::cout << "image saved under: " << filename << std::endl;
//			} 
